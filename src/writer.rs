...
pub const WRITER_I8: fn(&mut Cursor<&mut Vec<u8>>, i8) -> IOResult<()> = |cursor: &mut Cursor<&mut Vec<u8>>, value: i8| { cursor.write_i8(value)};
...
pub const WRITER_BOOL: fn(&mut Cursor<&mut Vec<u8>>, bool)  -> IOResult<()> = |cursor: &mut Cursor<&mut Vec<u8>>, v:bool| WRITER_U8(cursor,if (v){1} else {0});
